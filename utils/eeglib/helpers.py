...
from utils.eeglib.eeg import EEG
from utils.eeglib.preprocessing import bandPassFilter
import glob
import pandas as pd
...
# class CSVCustomHelper:
#     def __init__(self, directories, sep='\t', cutoff=[],*args, **kwargs):
#         self.data_frames = {}
#         self.lengths = {}
#         self.names = None  # Will be determined based on the first file read

#         # Process each directory and label
...
#             concatenated_df = pd.concat([pd.read_csv(file_name, sep=sep) for file_name in file_names])

#             # Optionally remove unwanted columns if all files have them
#             if 'index' in concatenated_df.columns and 'time stamp' in concatenated_df.columns:
#                 concatenated_df.drop(columns=['index', 'time stamp'], inplace=True)

#             self.data_frames[label] = concatenated_df
#             self.lengths[label] = len(concatenated_df)

#         combined_df = pd.concat(self.data_frames.values())
#         self.names = combined_df.columns.tolist()
#         self.data = combined_df.to_numpy().T
...
class CSVCustomHelper:
    def __init__(self, directories, sep='\t', cutoff=[], *args, **kwargs):
        ...
        for dir_key, label in directories.items():
            ...
            # Apply cutoffs if any are specified
            if cutoff:
                concatenated_df = self.apply_cutoffs(concatenated_df, cutoff)
    ...
    def apply_cutoffs(self, df, cutoffs):
        # Create a boolean mask for rows to keep, directly using the DataFrame's index
        mask = pd.Series(True, index=df.index)
        for start, stop in cutoffs:
            if stop == -1:
                mask.iloc[start:] = False
            else:
                mask.iloc[start:stop] = False
