...
for person, directories in all_directories.items():
    ...
    if person == '1713458084_Andrei':
        ...
        # post_stim2_idx = post_stim1_idx + length['VOR_1HZ_REST']
        # post_stim3_idx = post_stim2_idx + length['VOR_2HZ_REST']
        # post_stim4_idx = post_stim3_idx + length['VOR_3HZ_REST']
                
        # Dizziness onset
        post_stim1_idx = length['VOR_1HZ']
        post_stim2_idx = post_stim1_idx + length['VOR_2HZ']
        post_stim3_idx = post_stim2_idx + length['VOR_3HZ']
        post_stim4_idx = post_stim3_idx + length['VOR_4HZ']
    ...
    for _dizziness_ratio in dizziness_ratio:
        for _short in during_stim:
            for _base in stride_base:
                for  _stim in stride_stim:
                    vestibular_features = {}
                    
                    _ratio = dizziness_ratio[_dizziness_ratio]

                    after_stim_time = during_stim[_short]

                    window_stride_size_seconds_base = stride_base[_base]
                    window_stride_size_samples_base = window_stride_size_seconds_base*fs

                    window_stride_size_seconds_non_base = stride_stim[_stim]
                    window_stride_size_samples_non_base = window_stride_size_seconds_non_base*fs
                    
                    # Edit this functiopn to adapt the requirement
                    # 1. 10s / 20s / 30s, starting from the change in feedback from the subjects
                    # 2. 10s / 20s / 30s, staring 10-30s before the end of the vestibular stimulation  - On the way
                    # 3. The whole data during the vestibular stimulation, starting from the dizziness onset (based on the subjective feedback)
                    # middle1 = post_stim1_idx + int(length['VOR_1HZ']/2)
                    # middle2 = post_stim2_idx + int(length['VOR_2HZ']/2)
                    # middle3 = post_stim3_idx + int(length['VOR_3HZ']/2)
                    # middle4 = post_stim4_idx + int(length['VOR_4HZ']/2)
                    
                    middle1 = int(post_stim1_idx*_ratio)
                    middle2 = post_stim1_idx + int(length['VOR_2HZ']*_ratio)
                    middle3 = post_stim2_idx + int(length['VOR_3HZ']*_ratio)
                    middle4 = post_stim3_idx + int(length['VOR_4HZ']*_ratio)

                    # Only check the time during the stimulation, before and after the subjective feedback
                    def validBase(windowStart):
                        # return ((windowStart < middle1 - window_size_samples) or
                        # (windowStart > middle1 + after_stim_time*fs and windowStart < middle2 - window_size_samples) or
                        # (windowStart > middle2 + after_stim_time*fs and windowStart < middle3 - window_size_samples) or
                        # (windowStart > middle3 + after_stim_time*fs and windowStart < middle4 - window_size_samples) or
                        # (windowStart > middle4 + after_stim_time*fs)
                        # )
                        
                        return ((windowStart < middle1) or
                        (windowStart > post_stim1_idx and windowStart < middle2) or
                        (windowStart > post_stim2_idx and windowStart < middle3) or
                        (windowStart > post_stim3_idx and windowStart < middle4)
                        )
                    
                    print('EEG processing')
                    
                    eegHelper = [Helper(clean_eeg[i:i+1], 
                        sampleRate=fs, 
                    ...
                    for i in range(eeg_chans):
                        ...
                        while window_start < n_samples:
                            # Check if the current interval is the base data
                            valid_base = validBase(window_start)
                            
                            window_stride_size_samples = window_stride_size_samples_base if valid_base else window_stride_size_samples_non_base
                    ...
                    all_blink_events = extract_blink_rate_custom(eog_data.copy(), fs, window_size_samples, 
                                                                window_stride_size_samples_base, window_stride_size_samples_non_base, 
                                                                #  [post_stim1_idx, post_stim2_idx, post_stim3_idx, post_stim4_idx, post_stim5_idx], 
                                                                [middle1, middle2, middle3, middle4, post_stim1_idx, post_stim2_idx, post_stim3_idx],
                                                                after_stim_time*fs)

                    # eog_h_mean_val, eog_h_var, eog_h_skew, eog_h_kurt = extract_statistical_features(eog_data[0].copy(), window_size_samples, window_stride_size_samples)
                    # eog_v_mean_val, eog_v_var, eog_v_skew, eog_v_kurt = extract_statistical_features(eog_data[1].copy(), window_size_samples, window_stride_size_samples)
                    eog_h_mean_val, eog_h_var, eog_h_skew, eog_h_kurt = extract_statistical_features_custom(eog_data[0].copy(), window_size_samples, 
                                                                                                            window_stride_size_samples_base, 
                                                                                                            window_stride_size_samples_non_base, 
                                                                                                            #  [post_stim1_idx, post_stim2_idx, post_stim3_idx, post_stim4_idx, post_stim5_idx], 
                                                                [middle1, middle2, middle3, middle4, post_stim1_idx, post_stim2_idx, post_stim3_idx],
                                                                                                            after_stim_time*fs)
                    
                    eog_v_mean_val, eog_v_var, eog_v_skew, eog_v_kurt = extract_statistical_features_custom(eog_data[1].copy(), window_size_samples, 
                                                                                                            window_stride_size_samples_base, 
                                                                                                            window_stride_size_samples_non_base, 
                                                                #  [post_stim1_idx, post_stim2_idx, post_stim3_idx, post_stim4_idx, post_stim5_idx], 
                                                                [middle1, middle2, middle3, middle4, post_stim1_idx, post_stim2_idx, post_stim3_idx],
                    ...
                    eda_fs = 5
                    # eda_mean_val, eda_var, eda_skew, eda_kurt = extract_statistical_features(eda_data[::50].copy(), window_size_sec*eda_fs, window_stride_size_seconds*eda_fs)

                    eda_mean_val, eda_var, eda_skew, eda_kurt = extract_statistical_features_custom(eda_data.copy(), window_size_samples,  
                                                                                                            window_stride_size_samples_base, 
                                                                                                            window_stride_size_samples_non_base, 
                                                                #  [post_stim1_idx, post_stim2_idx, post_stim3_idx, post_stim4_idx, post_stim5_idx], 
                                                                [middle1, middle2, middle3, middle4, post_stim1_idx, post_stim2_idx, post_stim3_idx],
                    ...
                    # expanded_eda_mean_val = np.repeat(eda_mean_val[:, np.newaxis], segment_nums, axis=1)
                    # expanded_eda_var = np.repeat(eda_var[:, np.newaxis], segment_nums, axis=1)
                    # expanded_eda_skew = np.repeat(eda_skew[:, np.newaxis], segment_nums, axis=1)
                    # expanded_eda_kurt = np.repeat(eda_kurt[:, np.newaxis], segment_nums, axis=1)
                    ...
                    adjusted_subjective_feedback[:] = 1.0
                    # adjusted_subjective_feedback[post_stim1_idx:] = 0.
                    
                    adjusted_subjective_feedback[middle1: post_stim1_idx] = 2.
                    adjusted_subjective_feedback[middle2: post_stim2_idx] = 3.
                    adjusted_subjective_feedback[middle3: post_stim3_idx] = 4.
                    adjusted_subjective_feedback[middle4: ] = 5.
                    
                    # print(post_stim1_idx/fs, post_stim2_idx/fs, post_stim3_idx/fs, post_stim4_idx/fs)

                    
                    feedback = adjusted_subjective_feedback
                    true_labels = []
                    ...
                    while window_start < n_samples:
                        ...
                        # if not valid_base:
                        #     print(window_start/fs, window_size_samples, after_stim_time)
                        ...
                        # max_elements = [element for element, count in counter.items() if count == max_frequency]
                        # fb = max(max_elements)
                        # if fb==-1. and max_segment>1.0:
                        #     fb = max_segment
                        true_labels.append(max_segment)
                        
                        window_start += window_stride_size_samples
                        
                    true_labels = np.array(true_labels)
                    # expanded_lables = np.repeat(true_labels[:, np.newaxis], segment_nums, axis=1)
                    vestibular_features['Labels'] = true_labels
                    
                    
                    # Store data to features file
                    print('Featrues logging: ', len(vestibular_features['HR']))
                    ...
                    file_path = f'data/logging/{person}/stimulation_onset_{_ratio}_{_short}_{_base}_{_stim}.csv'
